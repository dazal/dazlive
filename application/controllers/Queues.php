...
class Queues extends CI_Controller {
	...
	{	
		...
		$date=new DateTime();
		$data['appntForDate'] = $date->format("d/m/Y");
		$data['isToday'] = 1;
		$data['queryresult'] = $this->Queue_model->getAllCustomerAppointments($date->format("d/m/Y"));
	...
	public function moveNextDay() {
		...
		$date = DateTime::createFromFormat('d/m/Y', $selDate);
		$date->modify('+1 day');

		$data['queryresult'] = $this->Queue_model->getAllCustomerAppointments($date->format('d/m/Y'));
		$data['appntForDate'] = $date->format('d/m/Y');
		$today=new DateTime();
		if($data['appntForDate'] == $today->format('d/m/Y')) {
			$data['isToday'] = 1;
		}	
		else {
			$data['isToday'] = 0;
